import { db, type ApiRequest, type Collection, type Environment, type HistoryEntry, type Workspace } from "@/services/db";
...
export async function exportCollection(collection: Collection): Promise<CollectionExport> {
  ...
  return { schema: "reqlo.collection", version: SCHEMA_VERSION, exportedAt: Date.now(), collection, requests };
}
...
export async function exportWorkspace(workspace: Workspace): Promise<WorkspaceExport> {
  ...
  return { schema: "reqlo.workspace", version: SCHEMA_VERSION, exportedAt: Date.now(), workspace, collections, requests, environments, history };
}
