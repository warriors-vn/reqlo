...
export function RequestBuilder({ request, onSend, sending }: Props) {
  ...
  const tabs = [
    { id: "params" as const, label: "Params", count: request.queryParams.filter(p => p.enabled).length || undefined },
    { id: "headers" as const, label: "Headers", count: request.headers.filter(h => h.enabled).length || undefined },
    { id: "body" as const, label: "Body", count: request.bodyType !== "none" ? "•" as const : undefined },
  ];
  ...
  return (
    <div className="flex flex-col border-b border-border bg-[var(--surface-elevated)]">
      ...
      <div className="flex items-center gap-2 px-4 pt-3">
        {nameEdit ? (
          <input
            ...
            onBlur={(e) => { renameRequest(request.id, e.target.value || "Untitled"); setNameEdit(false); }}
            onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); if (e.key === "Escape") setNameEdit(false); }}
            className="rounded-md border border-border bg-background px-2 py-0.5 text-sm font-medium focus-ring outline-none"
          />
        ) : (
          <button onClick={() => setNameEdit(true)} className="rounded px-1 py-0.5 text-sm font-medium hover:bg-accent">
      ...
      <div className="flex items-center gap-2 px-4 py-3">
        <div className="flex flex-1 items-stretch overflow-hidden rounded-lg border border-border bg-background shadow-sm focus-within:border-foreground/20 focus-within:shadow">
          <div className="relative">
            ...
            <div className={cn("pointer-events-none absolute bottom-0 left-2 right-2 h-[2px] rounded-full opacity-80", METHOD_BG[request.method])} />
          </div>
      ...
      <div className="flex items-center gap-1 border-b border-border px-3">
        {tabs.map(t => (
          ...
          >
            ...
            {tab === t.id && (
              <motion.div layoutId="reqtab" className="absolute -bottom-px left-1 right-1 h-[2px] rounded-full bg-primary" />
            )}
          </button>
        ))}
      </div>
      ...
      <div className="max-h-[40vh] min-h-[140px] overflow-auto px-4 py-3">
        {tab === "params" && <KVEditor list={request.queryParams} onChange={(queryParams) => updateRequest(request.id, { queryParams })} placeholder={["key", "value"]} />}
        {tab === "headers" && <KVEditor list={request.headers} onChange={(headers) => updateRequest(request.id, { headers })} placeholder={["Header", "Value"]} />}
        {tab === "body" && <BodyEditor request={request} />}
      </div>
    </div>
  );
}

function KVEditor({ list, onChange, placeholder }: { list: { id: string; key: string; value: string; enabled: boolean }[]; onChange: (v: typeof list) => void; placeholder: [string, string] }) {
  ...
  const add = () => onChange([...list, { id: Math.random().toString(36).slice(2), key: "", value: "", enabled: true }]);

  return (
    <div className="space-y-1">
      {list.length === 0 && (
        <div className="py-6 text-center text-xs text-muted-foreground">No entries. Add one to get started.</div>
      )}
      {list.map(item => (
        <div key={item.id} className="group flex items-center gap-2 rounded-md hover:bg-accent/40">
          ...
          />
          <button onClick={() => remove(item.id)} className="grid h-6 w-6 place-items-center rounded text-muted-foreground opacity-0 hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100">
            <X className="h-3 w-3" />
          </button>
        </div>
      ))}
      <button onClick={add} className="mt-1 flex items-center gap-1 rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-accent hover:text-foreground">
...
function BodyEditor({ request }: { request: ApiRequest }) {
  const updateRequest = useStore(s => s.updateRequest);
  return (
    <div className="space-y-2">
        ...
        {(["none", "json", "text"] as const).map(t => (
          <button
            ...
            className={cn(
              ...
              request.bodyType === t ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent/60",
            )}
          >
            {t}
          </button>
      ...
      {request.bodyType !== "none" && (
        <textarea
          ...
          className="block h-48 w-full resize-none rounded-md border border-border bg-background p-3 font-mono text-xs leading-relaxed outline-none focus:border-foreground/20"
        />
      )}
    </div>
  );
